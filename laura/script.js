...
function renderMissionPanel() {
  ...
  surpriseBox.innerHTML = `<span style="color:#6f624d;">Pulsa “Abrir sorpresa”.</span>`;
...
function renderProgress() {
  ...
  const points = completedCount * 120 + streak * 15;
  if (totalPointsEl) totalPointsEl.textContent = String(points);

  let rank = "Aprendiz del Río";
  if (completedCount >= 4) rank = "Guardiana del Nilo";
  else if (completedCount >= 3) rank = "Cartógrafa de Imperios";
  else if (completedCount >= 2) rank = "Cronista del Delta";
  if (rankNameEl) rankNameEl.textContent = rank;

  medals.forEach((medal, idx) => {
    const day = idx + 1;
    medal.classList.toggle("unlocked", !!state.completed[day]);
  });

  const today = todayISO();
  const todayLog = state.dailyLog[today];
  progressNote.textContent = (todayLog && todayLog.done)
    ? "Hoy ya has completado tu día. Tu camino sigue brillante."
    : "Hoy: microlectura + test + mapa mental + redacción breve + cierre “soy capaz”.";
}
...
function burstStars(target) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    requestAnimationFrame(() => {
      ...
      star.style.transform = `translate(${x}px, ${y}px) scale(${0.8 + Math.random() * 0.7})`;
...
function wireEvents() {
  checkQuizBtn.addEventListener("click", (e) => {
    ...
    if (g.correct === g.total) {
      ...
      burstStars(checkQuizBtn);
      showToast("¡Excelente respuesta!");
    } else {
  ...
  markDoneBtn.addEventListener("click", (e) => {
    ...
    burstStars(markDoneBtn);
    showToast("¡Misión completada! Medalla desbloqueada.");
  });
  ...
  sealBtn.addEventListener("click", (e) => {
    ...
    burstStars(sealBtn);
    showToast("Coronación completada 👑");
  });
